...
-\cs(125, 125, 0)${key_bind_dt} Lock DT Set :\cr ${toggle_lock_dt_set|OFF}
...
\cs(255, 115, 0)= State: \cr-\cs(125, 125, 0)${key_bind_pet_mode} Pet Mode :\cr ${pet_current_mode|TANK}-\cs(125, 125, 0)${key_bind_pet_style} Pet Style :\cr ${pet_current_style|NORMAL}-\cs(125, 125, 0) Combined State :\cr ${player_pet_state|Idle} 
    ...
    hub_mode_lte = [[ 
\cs(255, 115, 0)= Mode: \cr-\cs(125, 125, 0)${key_bind_idle} Idle Mode :\cr ${player_current_idle|Idle}-\cs(125, 125, 0)${key_bind_offense} Offense Mode :\cr ${player_current_offense|MasterPet}-\cs(125, 125, 0)${key_bind_hybrid} Hybrid Mode :\cr ${player_current_hybrid|Normal} 
]]

    hub_options_lte = [[ 
\cs(255, 115, 0)= Options: \cr-\cs(125, 125, 0)${key_bind_auto_maneuver} AutoMan :\cr ${toggle_auto_maneuver|OFF}-\cs(125, 125, 0)${key_bind_dt} \cs(125, 125, 0) AutoDep :\cr ${toggle_auto_deploy|OFF} 
]]
...
function validateTextInformation()
    ...
    if state.LockDT.value then
        main_text_hub.toggle_lock_dt_set = const_on
    else
        main_text_hub.toggle_lock_dt_set = const_off
    end
...
function job_buff_change(status, gain, eventArgs)
    ...
    if status == "sleep" and gain then
        equip(set_combine(sets.engaged.DT, {neck = "Opo-opo Necklace"}))
        eventArgs.handled = true
    elseif status == "doom" and gain then
        send_command("input /p I have befallen to ~~~DOOM~~~")
    elseif status == "doom" and gain == false then
        send_command("input /p I have avoided the grips of ~~~DOOM~~~")
    end
...
function job_state_change(stateField, newValue, oldValue)
        ...
        This could include OffenseMode, PetModeCycle -- etc
    ...
    elseif stateField == "Lock DT" then
